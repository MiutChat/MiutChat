 ...
 *   → Sets a signed, expiring miut_admin cookie (1hr) → Redirects to /
 *
 * MAINTENANCE TOGGLE:
...
const CRAWLER_UA      = /googlebot|bingbot|yandexbot|duckduckbot|slurp|baiduspider|facebot|facebookexternalhit|twitterbot|linkedinbot|ia_archiver/i;
const COOKIE_NAME     = 'miut_admin';
const COOKIE_MAX      = 3600;

// Routes that get a build-time prerendered snapshot for crawlers. This is
// dynamic rendering, not cloaking: the snapshot must contain the SAME
// visible text a real user sees after the SPA's JS finishes rendering —
// generated by prerendering the actual page (e.g. a headless-browser step
// in build.js), never hand-authored "bot content." If the snapshot doesn't
// exist yet (not built for this route), this silently falls through to the
// normal SPA response — never serves a broken or empty page to a crawler.
const PRERENDER_ROUTES = new Set(['/', '/index.html']);

async function tryServePrerendered(request, env, path) {
  if (!PRERENDER_ROUTES.has(path) || !env?.ASSETS) return null;
  const snapshotPath = path === '/' ? '/prerendered/index.html' : `/prerendered${path}`;
  try {
    const res = await env.ASSETS.fetch(new URL(snapshotPath, request.url));
    if (res.ok) {
      const headers = new Headers(res.headers);
      headers.set('X-Prerendered', '1'); // debugging aid — remove if it bothers you, harmless either way
      return new Response(res.body, { status: 200, headers });
    }
  } catch { /* ASSETS binding missing or snapshot not built — fall through */ }
  return null;
}
...
async function signAdminCookie(secret, expiresAt) {
  const sig = await hmacSign(secret, `${COOKIE_NAME}:${expiresAt}`);
  return `${expiresAt}.${sig}`;
}

async function verifyAdminCookie(value, secret) {
  if (!value || !secret) return false;
  const dot = value.indexOf('.');
  if (dot < 0) return false;
  const expiresAt = parseInt(value.slice(0, dot), 10);
  if (!Number.isFinite(expiresAt) || Date.now() > expiresAt) return false;
  const expectedSig = await hmacSign(secret, `${COOKIE_NAME}:${expiresAt}`);
  return timingSafeEq(value.slice(dot + 1), expectedSig);
}
...
export async function onRequest(ctx) {
  ...
  if (path === '/access') {
    ...
    const expiresAt = Date.now() + COOKIE_MAX * 1000;
    const cookieValue = await signAdminCookie(secret, expiresAt);
    return new Response(null, {
      status: 302,
      headers: {
        'Location': '/',
        'Set-Cookie': `${COOKIE_NAME}=${cookieValue}; Path=/; Max-Age=${COOKIE_MAX}; HttpOnly; Secure; SameSite=Strict`,
  ...
  if (isExempt) {
    // For crawlers on the SPA's dynamic entry route specifically, prefer a
    // prerendered snapshot when one exists — the SPA's real first paint is
    // near-empty until JS runs, which crawlers may not fully wait out.
    if (isCrawler) {
      const prerendered = await tryServePrerendered(request, env, path);
  ...
  // ══════════════════════════════════════════════════════════
  if (await verifyAdminCookie(getCookie(request, COOKIE_NAME), env?.ADMIN_ACCESS || '')) return next();
